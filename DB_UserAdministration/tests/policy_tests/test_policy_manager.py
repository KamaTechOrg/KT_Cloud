...
@pytest.fixture
def policy_manager():
    manager = PolicyManager('DB_UserAdministration/Test/PolicyTest/test.db')
    yield manager
...
class TestPolicyManager:
    ...
    def test_is_json_column_contains_key_and_value_with_contained(self, policy_manager):
        ...
        policy_manager.create(policy_test.name, policy_test.to_dict())
    ...
    def test_is_identifier_exist_with_existing(self, policy_manager):
        policy_manager.create('test_policy', Policy('test_policy').to_dict())
    ...
    def test_update(self, policy_manager):
        policy_test = Policy('test_policy')
        policy_manager.create(policy_test.name, policy_test.to_dict())
        policy_test.permissions.append(Permission(Action.READ, Resource.DATABASE, Effect.ALLOW))
