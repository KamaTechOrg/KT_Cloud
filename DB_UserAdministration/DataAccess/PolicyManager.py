...
class PolicyManager:
    ...
    def create(self, policy_id, metadata: Dict[str, Any]) -> None:
        '''Create a new policy in the database.'''
        try:
            self.db_manager.insert(
                table_name=self.table_name, object_id=policy_id, metadata=metadata)
    ...
    def update(self, policy_id: str, metadata: Dict[str, Any]) -> None:
        ...
        self.db_manager.update(self.table_name, {'metadata': json.dumps(
            metadata)}, f'id = "{policy_id}"')

    def get(self, policy_id: str) -> Dict[str, Any]:
        '''Retrieve a policy from the database.'''
        result = self.db_manager.select(
            self.table_name, ['metadata'], f'id = "{policy_id}"')
        if result:
